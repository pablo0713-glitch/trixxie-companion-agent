...
A self-hosted AI companion that lives simultaneously in **Discord** and **Second Life** (or OpenSimulator). Powered by **Claude (Anthropic)**, **OpenAI**, **Gemini**, **Grok**, **OpenRouter**, or any local model via **Ollama** or **LM Studio**. Personality, memory, tools, and platform behavior are fully configurable through a browser-based setup wizard — no code editing required.
...
4. At the top of the script, set:
   ```lsl
   string SERVER_URL = "https://your-tunnel-url-here.trycloudflare.com";
   string SECRET     = "your-bridge-secret";   // must match SL_BRIDGE_SECRET in .env
   ```
5. Save the script — it compiles automatically
...
If your agent's avatar uses **Cool VL Viewer**, you can install the Lua automation script for a native private IM experience — no `/42` channel command needed. The avatar shows a typing indicator while the model processes and replies in the IM window directly.

See [lua/README.md](lua/README.md) for setup. The LSL HUD is still required for sensor context.
